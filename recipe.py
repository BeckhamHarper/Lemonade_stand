...
def recipe_changer():
    if money.starting_question == "change the recipe":
        ...
        if int(recipe_sugar) > 4 or int(recipe_sugar) < 1 or int(recipe_sugar) > money.sugar:
        ...
        if int(recipe_ice_cups) > 4 or int(recipe_ice_cups) < 1 or int(recipe_ice_cups) > money.ice_cups:
