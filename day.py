...
def startDay(starting_question, ingredients, recipe1, cash, price_cup):
    global days
    if starting_question == "start day":
        ...
        cups_for_lemons = ingredients["lemons"] / int(recipe1["lemons for recipe"])
        cups_for_sugar = ingredients["sugar"] / int(recipe1["sugar for recipe"])
        cups_for_ice_cups = ingredients["ice_cups"] / int(recipe1["ice_cups for recipe"])
        ...
        for customer in customers:
            ...
            if (sweetness >= int(recipe1["sugar for recipe"]) and
                ice >= int(recipe1["ice_cups for recipe"]) and
                price >= price_cup):
                # Deduct ingredients used for one cup
                ingredients["lemons"] -= int(recipe1["lemons for recipe"])
                ingredients["sugar"] -= int(recipe1["sugar for recipe"])
                ingredients["ice_cups"] -= int(recipe1["ice_cups for recipe"])
                available_cups -= 1
                cups_sold += 1
                earnings += price_cup
